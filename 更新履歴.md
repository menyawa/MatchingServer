## 2021/07/26
---
・ServerのsupportMessageAsyncで最初に送られてくるプレイヤーデータを取得するよう修正  
・それを用いてタイムアウト時のメッセージを送るよう修正

・ServerのrunByClientMessageProgressAsyncでTimeOutのTypeのメッセージを受け取った際、通常の切断処理を行うよう修正  
・ServerのrunByClientMessageProgressAsyncでTimeOutのTypeのメッセージを受け取った際、まだルームにいるなら退出処理を行うよう修正  

・ServerのsupportMessageAsyncでタイムアウトして強制切断する場合、クライアント側で既に切断されていることを考慮し、WebSocketExceptionをtry-catchするよう修正

・ServerのsupportMessageAsyncでメインループを抜けた後で接続している場合、Abortを用いて強制切断を行うよう修正

・Serverでタイムアウト切断時に入出力を中止するよう修正

## 2021/07/25
---
・ServerのgetReceiveMessageAsyncでエンドポイントCloseのメッセージを無視するよう修正  
・getReceiveMessageAsyncで受信したメッセージがnullの場合、次の受信を開始して次のループにcontinueするよう修正

・通常切断時、タイムアウト切断時に切断ができていなかった問題を修正  
・それに伴い、エンドポイントCloseのメッセージがServerから届いた際、Disconnectのメッセージに直して受信するよう修正  
・また、ServerのgetReceiveMessageAsync、sendMessageAsyncでisConnectedを用い、接続されていないWebSocketが渡された場合デバッグメッセージを表示してArgumentExceptionを投げるよう修正  
・ServerのisConnectedを切断要求メッセージのことを考慮した判定に修正  
・ServerのrunByClientMessageProgressAsyncでの切断時、ルームにまだいるなら退室処理を行うよう修正  
